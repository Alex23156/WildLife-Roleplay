...
function UILib:AddBuyersTab(name, decalID)
    ...
    -- REGISTRO GLOBAL DE BUYERS AÑADIDOS
    if not getgenv().AddedBuyerIds then
        getgenv().AddedBuyerIds = {}
    end
    
    if not getgenv().ApiBuyerFrames then
        getgenv().ApiBuyerFrames = {}
    end
    ...
    local function NewTarget(targetPlayer, initialGoal, initialPending)
        local oderId = targetPlayer.UserId
        
        -- VERIFICAR SI YA ESTÁ AÑADIDO
        if getgenv().AddedBuyerIds[oderId] then
            return nil
        end
        ...
        -- GUARDAR REFERENCIA DEL FRAME (para API buyers)
        ...
        local isFromAPI = (initialGoal and initialGoal > 0)
        Players.PlayerRemoving:Connect(function(player)
            -- Solo limpiar si es buyer de API
            if getgenv().ApiBuyerFrames[player.UserId] then
                local frame = getgenv().ApiBuyerFrames[player.UserId]
                if frame and frame.Parent then
                    frame:Destroy()
                end
                getgenv().ApiBuyerFrames[player.UserId] = nil
                getgenv().AddedBuyerIds[player.UserId] = nil
            end
            -- Los buyers manuales no se tocan
        end)
        local canDelete = not isFromAPI -- Solo puede borrar si NO es de API

        -- Si es de API, ocultar botón de eliminar hasta que complete
        if isFromAPI then
            removeBtn.Visible = false
            removeBtn.ImageTransparency = 0.5
        end

        removeBtn.MouseButton1Click:Connect(function()
            if canDelete then
                -- LIMPIAR DEL REGISTRO AL ELIMINAR
                if getgenv().AddedBuyerIds then
                    getgenv().AddedBuyerIds[targetPlayer.UserId] = nil
                end
                if getgenv().ApiBuyerFrames then
                    getgenv().ApiBuyerFrames[targetPlayer.UserId] = nil
                end
                frame:Destroy()
            end
        end)
        ...
        -- Si viene con goal inicial de la API (auto-add)
        if initialGoal and initialGoal > 0 then
            goalValue = initialGoal
            earnedSoFar = initialPending or 0
            goalStartCurrency = currencyValue.Value - earnedSoFar
            startCurrency = currencyValue.Value
            currentGoalActive = true
            goalBox.Text = FormatNumber(goalValue)
            goalBox.TextEditable = false
            goalBox.Active = false  -- Bloquear clicks
            goalBox.Selectable = false  -- No seleccionable
            goalBoxStroke.Color = Color3.fromRGB(0, 200, 100)
            
            -- Bloquear cualquier intento de focus
            goalBox.Focused:Connect(function()
                goalBox:ReleaseFocus()
            end)
        end
        ...
        local function updateLabels()
            ...
            if diff > 0 then
                ...
                -- Si es cliente de API, actualizar progreso en la base de datos
                if isFromAPI and getgenv().ActualizarClienteAPI and getgenv().KamaikScriptKey then
                    task.spawn(function()
                        getgenv().ActualizarClienteAPI(getgenv().KamaikScriptKey, targetPlayer.UserId, earnedSoFar)
                    end)
                end
            end
        ...
        goalBox.FocusLost:Connect(function(enterPressed)
            ...
            if parsed > 0 then
                ...
                if not goalSetSent then
                    ...
                    local goalSetMsg = getgenv().AutofarmSettings.Shouts["Goal Set"] or "{buyer} Your goal has been set, start picking up your DHC"
                    ...
                    if getgenv().BringOnGoalSet and targetPlayer and targetPlayer.Parent then
        ...
        task.spawn(function()
        local buyerLastPosition = nil
        local buyerInactivityTimer = 0
        local inactivityLimit = 150
        local inactivityWarningSent = false
        ...
        while frame.Parent do
            if goalValue > 0 then
                ...
                if not goalSent and remaining == 0 then
                    ...
                    -- Permitir borrar una vez completado
                    canDelete = true
                    removeBtn.Visible = true
                    removeBtn.ImageTransparency = 0
                    -- Enviar vouch via API (kamaik)
                    task.spawn(function()
                    ...
                    if initialGoal and initialGoal > 0 then
                        ...
                        pcall(function()
                            ...
                            if requestFunc then
                                ...
                                -- Multi Mode: agregar sellerDiscordId
                                if getgenv().KamaikSellerDiscordId then
                                    body.sellerDiscordId = getgenv().KamaikSellerDiscordId
                                end
                    ...
                    if getgenv().bringlocationgoalreached and targetPlayer and targetPlayer.Parent then
                        ...
                        task.spawn(function()
                            ...
                            if tp and tp.Parent then
                                game.ReplicatedStorage.MainEvent:FireServer("VIP_CMD", "Summon", tp)
                            end
                    ...
                    elseif getgenv().BringOnGoalFinished and targetPlayer and targetPlayer.Parent then
                        game.ReplicatedStorage.MainEvent:FireServer("VIP_CMD", "Summon", targetPlayer)
                    end
                ...
                -- ═══ INACTIVITY CHECK PARA ESTE BUYER ═══
                if not goalSent and remaining > 0 and targetPlayer and targetPlayer.Character then
                    local buyerRoot = targetPlayer.Character:FindFirstChild("HumanoidRootPart")
                    if buyerRoot then
                        local currentPos = buyerRoot.Position
                        if buyerLastPosition then
                            if (currentPos - buyerLastPosition).Magnitude < 0.5 then
                                buyerInactivityTimer = buyerInactivityTimer + 1
                            else
                                buyerInactivityTimer = 0
                                inactivityWarningSent = false  -- Reset si se movió
                            end
                        end
                        buyerLastPosition = currentPos
                        
                        if buyerInactivityTimer >= inactivityLimit and not inactivityWarningSent then
                            local msg = getgenv().AutofarmSettings.Shouts["Inactivity Warning"] or "{buyer}, please pick up your remaining DHC"
                            msg = msg:gsub("{buyer}", "@" .. targetPlayer.Name)
                            game.ReplicatedStorage:WaitForChild("MainEvent"):FireServer("Shout", msg)
    ...
    addButton.MouseButton1Click:Connect(function()
        ...
        if targetPlayer then
            -- VERIFICAR SI YA ESTÁ AÑADIDO
            if getgenv().AddedBuyerIds and getgenv().AddedBuyerIds[targetPlayer.UserId] then
                addUserBox.Text = ""
    ...
    local logsWebhook = getgenv().AutofarmSettings.Webhooks.Logs
    local joinedPlayers = {}
    ...
    -- Exponer función para auto-añadir buyers desde API
    getgenv().AutoAddBuyerWithGoal = function(targetPlayer, goalAmount, pendingAmount)
        ...
        NewTarget(targetPlayer, goalAmount, pendingAmount)
        return true
    end
end
