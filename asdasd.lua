...
local function attemptGetLMG(timeoutSeconds)
    ...
    -- offset arriba de 3 studs
    local targetCFrame = CFrame.new(lmgTP + Vector3.new(0, 3, 0))

    -- activar vuelo hacia esa posición
    pcall(function()
        activarVueloNuevo(targetCFrame)
    end)
    local startTime = tick()
    
    while not tieneTodasLasArmas() and (tick() - startTime) < timeoutSeconds do
        task.wait(1)
        clickLMG()
    end
    
    if tieneTodasLasArmas() then
        return true
    else
        return false
    end
...
local function equiparLMG()
    ...
    -- Si ya tiene LMG equipada, devolver true
    local lmgTool = char:FindFirstChild("[LMG]")
    if lmgTool then return true end
    
    -- Buscar LMG en la mochila
    local backpackLMG = player.Backpack:FindFirstChild("[LMG]")
    if backpackLMG then
        charHumanoid:EquipTool(backpackLMG)
        task.wait(0.5) -- Esperar a que se equipe
        return char:FindFirstChild("[LMG]") ~= nil
    end
...
local function obtenerBalas()
    ...
    if tool and tool.Name == "[LMG]" then
...
local function comprarAmmo()
    ...
    if humanoid then
        ...
        task.wait(0.3) -- Pequeña pausa para que procese el desequipamiento
    end
...
local function verificarYRecargarLMG()
    ...
    -- Verificar si tiene LMG equipada
    if not equiparLMG() then
        Status.Text = "Anti-Jail: Getting LMG..."
        if not attemptGetLMG() then
            return false
        end
        task.wait(0.5)
        if not equiparLMG() then
...
local function startAutoShoot(target)
    ...
    connection = RunService.Heartbeat:Connect(function()   
        ...
        local lmgTool = char:FindFirstChild("[LMG]")
        if not lmgTool or not lmgTool:FindFirstChild("Handle") then
            local backpackLMG = player.Backpack:FindFirstChild("[LMG]")
            if backpackLMG then
                backpackLMG.Parent = char
                task.wait(0.1)
                return
            else
                isAutoShooting = false
        ...
        local ok, err = pcall(function()
            mainEvent:FireServer(
                "ShootGun",
                lmgTool.Handle,
                lmgTool.Handle.CFrame.Position,
                head.Position,
                head,
                Vector3.new(0, 0, -1)
            )
...
local function superDisparo(target)
    ...
    -- VERIFICAR QUE EL CAJERO ESTÉ VIVO ANTES DE DISPARAR
    ...
    -- Equipar LMG si no está equipada
    local lmgTool = char:FindFirstChild("[LMG]")
    if not lmgTool then
        local backpackLMG = player.Backpack:FindFirstChild("[LMG]")
        if backpackLMG then
            backpackLMG.Parent = char
            task.wait(0.2)
            lmgTool = char:FindFirstChild("[LMG]")
        else
            return false
        end
    end
    ...
    -- ERIFICAR MUNICIÓN ANTES DE EMPEZAR A DISPARAR
    local balas = obtenerBalas()
    if not balas or balas <= 0 then
    ...
    local maxWaitTime = 15 -- Máximo 15 segundos disparando
    local startTime = tick()
    
    -- LOOP MEJORADO CON VERIFICACIONES ADICIONALES
    while target and target.Parent and target:FindFirstChild("Humanoid") and target.Humanoid.Health > 0 do
        ...
        -- VERIFICAR SI CURRENTTARGET FUE LIMPIADO (por auto-reload)
        if not currentTarget then
            Status.Text = "Target cleared during reload"
            break
        end
        
        -- VERIFICAR MUNICIÓN DURANTE EL DISPARO
        local balasActuales = obtenerBalas()
        if balasActuales and balasActuales <= 0 then
            Status.Text = "Out of ammo during shooting"
            break
        end
    ...
    -- Detener disparo y desequipar arma
    stopAutoShoot()
    if charHumanoid then
        charHumanoid:UnequipTools()
    end
    
    -- VERIFICAR SI EL CAJERO MURIÓ REALMENTE
...
task.spawn(function()
    ...
    if isJailed then
        ...
        -- Esperar hasta confirmar que salió de jail
        ...
        -- Si después de 30 segundos sigue en jail, intentar de nuevo
    ...
    -- PRIORIDAD 2: Inicializar sistema LMG solo después de confirmar que NO está en jail
    Status.Text = "Initializing LMG System..."
    
    if not tieneTodasLasArmas() then
        Status.Text = "Getting LMG..."
        if attemptGetLMG() then
            Status.Text = "LMG obtained!"
        end
    end
...
task.spawn(function()
    while true do
        ...
        if target and target:FindFirstChild("Head") and target:FindFirstChild("Humanoid") then
            ...
            --  PRIMERO EQUIPAR EL ARMA
            if Status.Text == "Resuming farm..." then
                task.wait(1) -- Dar tiempo a autoReloadSystem
            end

            Status.Text = "Equipping LMG..."
            if not equiparLMG() then
                Status.Text = "No LMG available - getting weapon..."
                if attemptGetLMG() then
                    Status.Text = "LMG obtained, equipping..."
                    if not equiparLMG() then
                        Status.Text = "Failed to equip LMG"
                        Kamaik.AutoFarm.lastTarget = nil
                        task.wait(2)
                        continue
                    end
                else
                    Status.Text = "Failed to get LMG"
                    Kamaik.AutoFarm.lastTarget = nil
                    task.wait(5)
                    continue
                end
            ...
            --  VERIFICAR MUNICIÓN CON EL ARMA EQUIPADA
            task.wait(0.3) -- Pequeña pausa para asegurar que el arma está equipada
            local balas = obtenerBalas()
            if not balas or balas <= 0 then
                Status.Text = "No ammo - triggering reload..."
                --  FORZAR RELOAD INMEDIATO
                recargar()
                task.wait(2)
                
                -- Verificar si se recargó
                local nuevasBalas = obtenerBalas()
                if not nuevasBalas or nuevasBalas <= 0 then
                    Status.Text = "Reload failed - buying ammo..."
                    comprarAmmo()
                    task.wait(3)
                    recargar()
                    task.wait(1)
                ...
                local balasFinales = obtenerBalas()
                if not balasFinales or balasFinales <= 0 then
            ...
            --  VERIFICAR SI EL TARGET SIGUE SIENDO VÁLIDO DESPUÉS DEL DISPARO
