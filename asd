        ...
        if FASE_ROMPIENDO then
            ...
            if target and target:FindFirstChild("Head") and target:FindFirstChild("Humanoid") then
                ...
                if humanoid then
                    local char = player.Character
                    if char then
                        -- Determinar arma a usar
                        local armaAUsar = armaActual or "[AUG]"  -- ✅ Usar armaActual si existe
                        if AUG_GASTADA then
                            armaAUsar = "[LMG]"
                            armaActual = "[LMG]"  -- ✅ Actualizar armaActual también
                        end
                    ...
                    if esCajeroEspecial then
                        ...
                        -- ✅ SOLO EN CAJEROS NORMALES VERIFICAR BALAS Y CAMBIAR ARMA
                        ...
                        -- Cajero normal: guardar para después
            ...
            -- PREPARACIÓN INICIAL ANTES DE RECOGER
            ...
            if char then
                ...
                if equiparArma("[LMG]") then
                    task.wait(0.3)
                    -- Intentar recargar (sin importar si tiene balas o no)
                    recargar()
                    task.wait(1)
                end
                
                -- Desequipar arma para recoger
                local humanoid = char:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    humanoid:UnequipTools()
                end
            ...
            while i <= #cajerosRotos do
                ...
                -- Teleportarse directamente a recoger
                ...
                -- 4. Reactivar vuelo y recoger dinero
                Status.Text = "Collecting (" .. i .. "/" .. #cajerosRotos .. ")"
                activarVueloNuevo(CFrame.new(posVuelo))
                task.wait(0.2)
                local intentos = 0
            ...
            -- DESPUÉS DE RECOGER TODO: Recargar ambas armas
            ...
            if char then
                ...
                -- 3️⃣ SI ALGUNA NECESITA MUNICIÓN, IR A COMPRAR
