...
local function hayVaultsDisponibles()
    return contarVaultsVivos() > 0
end

-- Función para comprar granadas
...
local function robarVault()
    ...
    -- PASO 6: Asegurar armas equipadas
    Status.Text = "VAULT: Preparing weapons..."
    ...
    -- PASO 7: Atacar los 4 VAULTs (ID: 1, 2, 3, 4)
    local vaultIDs = {1, 2, 3, 4}
    local vaultsOffsets = {
        offsetX = 0,
        offsetY = 12.50,
        offsetZ = 7.5
    ...
    for _, vaultID in ipairs(vaultIDs) do
        local vault = encontrarVaultPorID(vaultID)
        
        if vault and vault:FindFirstChild("Head") and vault:FindFirstChild("Humanoid") and vault.Humanoid.Health > 0 then
            Status.Text = "VAULT: Attacking Vault " .. vaultID .. "/4..."
            
            -- Volar al vault con offsets personalizados
            local vaultPos = vault.Head.Position
            local posicionVuelo = Vector3.new(
                vaultPos.X + vaultsOffsets.offsetX,
                vaultPos.Y + vaultsOffsets.offsetY,
                vaultPos.Z + vaultsOffsets.offsetZ
            )
            ...
            -- Usar superDisparo (igual que con cajeros)
            local resultado = superDisparo(vault)
            
            if resultado then
                Status.Text = "VAULT " .. vaultID .. " destroyed! Collecting..."
                
                -- Desequipar armas para recoger
                ...
                -- Posición para recoger dinero (4 studs abajo)
                local posRecoger = Vector3.new(
                    vaultPos.X + vaultsOffsets.offsetX,
                    vaultPos.Y + vaultsOffsets.offsetY - 4,
                    vaultPos.Z + vaultsOffsets.offsetZ
                )
                ...
                Status.Text = "VAULT " .. vaultID .. " money collected!"
                
                -- Esperar un poco antes del siguiente vault
                if vaultID < 4 then
                    teleportWithRandomFall()
                    task.wait(2)
                end
            else
                Status.Text = "VAULT " .. vaultID .. " failed to destroy"
            end
        else
            Status.Text = "VAULT " .. vaultID .. " not found or already dead"
        end
    end
    
    -- PASO 8: Finalizar y volver al farm normal
    Status.Text = "VAULT: Robbery complete!"
    teleportWithRandomFall()
    task.wait(2)
    
    -- Recargar armas antes de volver al farm
...
local function serverHop()
    ...
    repeat
        ...
        for _, v in ipairs(Servers.data) do
            ...
            if v.id ~= _id and v.playing < v.maxPlayers then
                ...
                if not combinedBlacklist[tostring(v.id)] then
                    -- Dejar un margen de 2 jugadores para evitar "Server is full"
                    if (v.maxPlayers - v.playing) >= 2 then
                        Status.Text = "Joining server (" .. serversChecked .. " checked)..."
                        
                        local s, r = pcall(TPS.TeleportToPlaceInstance, TPS, _place, v.id, Player)
                        
                        if s then 
                            -- Teleport exitoso, esperar
                            task.wait(10)
                            return
                        else
        ...
        -- Si revisamos muchos servidores sin éxito, reintentar desde el inicio
        if attemptCount >= maxAttempts then
    ...
    -- Si llegamos aquí sin encontrar servidor, reintentar después de un tiempo
    Status.Text = "No suitable server found, retrying..."
    task.wait(5)
    serverHop()
end
...
task.spawn(function()
    ...
    while true do
        ...
        -- ✅ PRIORIDAD 1: VERIFICAR VAULT ANTES QUE NADA
        if not jailProcessActive and not vaultRobberyInProgress then
...
-- Limpiar Backpack (EXCEPTO [LMG])
player.Backpack.ChildAdded:Connect(function(child)
    task.wait(0.1)
    if child.Name ~= "[LMG]" and child.Name ~= "[AUG]" and not child.Name:find("LMG") and not child.Name:find("AUG") then
        pcall(function() child:Destroy() end)
    end
end)

-- Limpiar items existentes en Backpack al inicio (EXCEPTO [LMG])
for _, item in ipairs(player.Backpack:GetChildren()) do
    if item.Name ~= "[LMG]" and item.Name ~= "[AUG]" and not item.Name:find("LMG") and not item.Name:find("AUG") then
